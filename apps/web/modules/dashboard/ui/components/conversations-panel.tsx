    ...
    return (
        <div className="flex h-full w-full flex-col bg-background text-sidebar-foreground">
            <div className="flex flex-col gap-3.5 border-b-2 p-2">
                <Select
                    defaultValue="all"
                    onValueChange={(value: Doc<"conversations">["status"] | "all") => setStatus(value)}
                    value={status}
                >
                    ...
                    {conversations.results.map((conversation) => {
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    "relative flex cursor-pointer items-start border-b p-4 py-5 text-sm leading-tight hover:bg-accent hover:text-accent-foreground",
                                    pathname === `/conversations/${conversation._id}` && "bg-accent text-accent-foreground"
                                )}
                            >
                                <div
                                    className={cn("-translate-y1/2 absolute top-1/2 left-0 h-[64%] w-1 rounded-r-full bg-neutral-300 opacity-0 transition-opacity",
                                    ...
                                    <div className="mt-1 flex items-center justify-between gap-2">
                                        <div className=" flex w-0 grow items-center gap-1">
                                            {isLastMessageFromOperator && (
                                                <CornerUpLeftIcon className="size-3 text-muted-foreground shrink-0" />
                                            )}
                                            <span className={cn(
            ...
            <div className="relative flex w-full min-w-0 flex-col p-2">
                <div className="w-full space-y-2">
                    {Array.from({ length: 8 }).map((_, index) => (
                        <div className="flex items-start gap-3 rounded-lg border-b p-4 py-5" key={index}>
